...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Handle 'Others' in preferred areas
    ...
    if (in_array("Others", $preferred_areas_arr) && $preferred_areas_other_text !== '') {
        foreach ($preferred_areas_arr as $k => $v) {
            if ($v === "Others") {
                $preferred_areas_arr[$k] = "Others: " . htmlspecialchars($preferred_areas_other_text);
            }
        }
    }
    ...
    if ($stmt) {
        ...
        if ($stmt->execute()) {
            // *** EMAIL INTEGRATION ***
            // After successful database insertion, send the notification email.
            $recipientName = $first_name . ' ' . $last_name;
            sendVolunteerApplicationNotice($email, $recipientName);
            ...
            <?php
            exit; // Stop script execution after showing the success page
        } else {
    ...
    <div class="modal-content">
        ...
        <form method="POST" enctype="multipart/form-data" id="volunteerForm">
            ...
            <div class="form-grid">
                ...
                <div class="form-group">
                    <label for="urgent_consult">Available for urgent consultations?</label>
            ...
            <div class="form-group full-width">
                ...
                <textarea name="motivation" id="motivation" rows="4" required placeholder="Share your motivation here..."></textarea>
                <small style="color:#555;display:block;margin-top:4px; font-family: 'Inter', Arial, sans-serif;">
                    We’d love to know what drives you to offer your time and expertise.
                </small>
            </div>

            <div class="form-divider"></div>
            <div class="checkbox-group">
                <label><input type="checkbox" name="info_certified" required> By checking this box, I certify that all the information I have provided is true and accurate.</label>
            </div>
            <div class="checkbox-group">
                <label><input type="checkbox" name="consent_check" required> I voluntarily consent to a background check for verification purposes.</label>
            </div>
            <div class="checkbox-group">
                <label>
                    <input type="checkbox" name="agree_terms" required>
                    I agree to the <a href="#terms-popup" style="color:#1b5fa7;text-decoration:underline;margin-left:4px;">Terms and Conditions</a>.
                </label>
            </div>
            <div style="text-align: right; margin-top: 24px;">
                <button type="button" class="back-btn" style="margin-right:12px; background:#b48be3;" onclick="window.location.href='index.html';">Back</button>
...
<style>
.overlay { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.25); z-index: 9999; display: none; }
.overlay:target { display: flex; align-items: center; justify-content: center; }
.popup { background: #fff; border-radius: 18px; box-shadow: 0 8px 32px rgba(0,0,0,0.15); padding: 48px 64px 36px 64px; max-width: 720px; width: 95%; margin: 20px; position: relative; animation: fadeIn 0.3s; }
.back-btn { display: inline-block; background: #5A2675; color: #fff !important; border: none; border-radius: 6px; padding: 10px 32px; font-size: 16px; font-weight: 500; text-decoration: none; margin-top: 10px; cursor: pointer; box-shadow: 0 2px 8px rgba(90,38,117,0.2); }
.back-btn:hover { background: #391053; }
</style>
